...
export const timeUpdate = (tick: number, timeWarp: null | number = null) => {
    ...
    if (activeAll.includes(4)) {
        ...
        if (activeAll.includes(5)) {
            ...
            if (research >= 4 && (player.challenges.active !== 0 || player.buildings[6][1].current.moreOrEqual('7'))) { gainBuildings(4, 5, passedSeconds); } //Quasi-stars
...
export const calculateEffects: calculateEffectsType = {
    ...
    effectiveGoals: () => {
        ...
        if (player.inflation.vacuum) {
            ...
            if (player.tree[1][4] >= 1) { goals += global.accretionInfo.effective / 2; }
        }
        return goals;
    },
    ...
    dischargeBase: (research = player.researches[1][4]) => {
        ...
        if (player.challenges.active === 0) { base = (base + player.tree[1][3] / 2) ** 0.5; }
        return base;
    },
    ...
    strangeGain: (interstellar, quarks = true) => {
        ...
        } else { base *= 1.4 ** player.strangeness[6][1]; }
...
export const assignBuildingsProduction = {
    globalSpeed: (): number => {
        ...
        let speed = (1.4 ** player.strangeness[6][0]) * (calculateEffects.T0Inflation1() ** tree[0][1]) * effectsCache.T0Inflation3 * (1.4 ** tree[1][0]);
        if (tree[1][2] >= 1) {
            speed *= 2 ** tree[0][0];
        } else {
    ...
    S1Build6: (): Overlimit => {
        ...
        effectsCache.tritium.setValue(assignBuildingsProduction.S1Build5()).max('1').log(calculateEffects.S1Extra1()).multiply(multiplier);
    ...
    S2Build1: (): Overlimit => {
        ...
        global.buildingsInfo.producing[2][1].setValue((vacuum ? 2 : 8e-4) * ((player.challenges.active === 0 && player.challenges.super ? 2 : 3) ** global.vaporizationInfo.S2Research0) * (2 ** player.strangeness[2][0])).multiply(...multiplierList);
        if (vacuum) { return global.buildingsInfo.producing[2][1].max('1'); }
        return global.buildingsInfo.producing[2][1];
    },
    /** Puddles, visual will assign effects for all Structures (minus Drops) */
    S2Build2: (visual = false): number => {
        const producings = global.buildingsInfo.producing[2];
        const structures = player.buildings[2];
        if (structures[2].true < 1) {
            const rain = calculateEffects.S2Extra1(player.researchesExtra[2][1]);
            if (!visual) { return (rain - 1) * calculateEffects.S2Extra2(rain); }
            producings[2].setValue((rain - 1) * calculateEffects.S2Extra2(rain));
        }
        ...
        const producing5 = Math.max(start * structures[5].current.toNumber() * calculateEffects.S2Extra2(rain) * flow, 1);
        const producing4 = Math.max(start * structures[4].current.toNumber() * flow, 1);
        const producing3 = Math.max(start * structures[3].current.toNumber() * flow, 1);

        let multiplier = (inVoid ? 6e-4 : 4.8) * structures[2].current.toNumber() * calculateEffects.clouds() * producing3 * producing4 * producing5 * producing6 * effectsCache.S2Upgrade3 * effectsCache.S2Upgrade4 * ((inVoid && player.challenges.super ? 1.4 : 2) ** global.vaporizationInfo.S2Research1) * rain * ((player.inflation.vacuum ? 1.8 : 1.6) ** player.strangeness[2][1]);
        ...
        if (visual) {
            producings[6].setValue(producing6);
            producings[5].setValue(producing5);
            producings[4].setValue(producing4);
            producings[3].setValue(producing3);
            if (structures[2].true >= 1) { producings[2].setValue(multiplier); }
        }
        return multiplier;
    },
...
export const assignResetInformation = {
    ...
    newClouds: (): number => {
        ...
        return (global.vaporizationInfo.get = (player.vaporization.clouds ** (1 / softcap) + new Overlimit(player.buildings[2][1][player.researchesExtra[2][0] >= 1 ? 'total' : 'current']).toNumber() / calculateEffects.S2Upgrade2()) ** softcap - player.vaporization.clouds);
    },
    maxRank: () => {
        if (player.inflation.vacuum) {
    ...
    solarHardcap: () => {
        ...
        if (player.tree[1][4] >= 3) { global.collapseInfo.solarCap *= 1.01 ** global.accretionInfo.effective; }
    },
...
export const buyUpgrades = (upgrade: number, stageIndex: number, type: 'upgrades' | 'researches' | 'researchesExtra' | 'researchesAuto' | 'ASR' | 'elements', auto = false): boolean => {
    ...
    if (type === 'upgrades') {
        ...
        if (!auto && globalSave.SRSettings[0]) { getId('SRMain').textContent = `Level increased ${level[upgrade] >= pointer.max[upgrade] ? 'and maxed at' : 'to'} ${format(level[upgrade])} for the '${pointer.name[upgrade]}' ${type === 'researches' ? 'Stage' : ['Energy', 'Clouds', 'Rank', 'Collapse', 'Galaxy'][player.stage.active - 1]} Research`; }
...
export const buyStrangeness = (upgrade: number, stageIndex: number, type: 'strangeness' | 'inflations', auto = false): boolean => {
    ...
    if (type === 'strangeness') {
        ...
        assignBuildingsProduction.strange0();
        if (!auto && globalSave.SRSettings[0]) { getId('SRMain').textContent = `Level increased ${strangeness[upgrade] >= pointer.max[upgrade] ? 'and maxed at' : 'to'} ${format(strangeness[upgrade])} for the '${pointer.name[upgrade]}' Strangeness from ${global.stageInfo.word[stageIndex]} section`; }
        ...
        if (stageIndex === 0) {
            ...
            if (upgrade === 2) {
                ...
                assignMilestoneInformation(0, 1);
            } else if (upgrade === 4) {
                calculateMaxLevel(0, 4, 'researches', true);
            }
        }
    ...
    if (type === 'researches') {
        if (stageIndex === 2) {
            ...
            if (research === 0) {
                ...
                if (player.inflation.vacuum && player.tree[1][4] >= 2) { max += Math.floor(global.accretionInfo.effective); }
            } else if (research === 1) {
...
export const stageResetUser = async() => {
    ...
    if (globalSave.SRSettings[0]) {
        ...
        if (player.stage.true >= 5) {
            ...
            const strangelets = player.strangeness[5][8] >= 1 ? calculateEffects.strangeGain(active >= 4, false) : 0;
...
const vaporizationResetCheck = (clouds = null as number | null): boolean => {
    ...
    if (player.upgrades[2][2] < 1 || global.vaporizationInfo.get <= 0) { return false; }
    ...
    if (clouds !== null) {
        ...
        if ((calculateEffects.clouds(true) / calculateEffects.clouds()) * (rainAfter / rainNow) * storm < vaporization.input[0]) { return false; }
        vaporizationReset();
    }
    return true;
};
export const vaporizationResetUser = async() => {
    ...
    if (player.toggles.confirm[2] !== 'None') {
        ...
        const rainAfter = calculateEffects.S2Extra1(player.researchesExtra[2][1], true);
        const storm = calculateEffects.S2Extra2(rainAfter) / calculateEffects.S2Extra2(rainNow);
        if ((calculateEffects.clouds(true) / calculateEffects.clouds()) * (rainAfter / rainNow) * storm < 2) {
    ...
    if (globalSave.SRSettings[0]) {
        getId('SRMain').textContent = `Caused Vaporization for ${format(global.vaporizationInfo.get)} Clouds${player.vaporization.clouds > 0 ? `, +${format(global.vaporizationInfo.get / player.vaporization.clouds * 100)}` : ''}%`;
    }
    vaporizationReset();
    numbersUpdate();
};
...
const vaporizationReset = (autoClouds = null as number | null) => {
    ...
    if (autoClouds !== null) {
        vaporization.clouds += global.vaporizationInfo.get * autoClouds / 40;
    } else {
...
const endResetCheck = (): boolean => {
    if (player.upgrades[6][0] !== 1 || player.buildings[6][1].true < 1) { return false; }
    return true;
};
export const endResetUser = async() => {
    ...
    if (player.toggles.confirm[6] !== 'None') {
        if (player.inflation.vacuum && calculateBuildingsCost(1, 6).toNumber() <= calculateEffects.mergeScore()) {
    ...
    if (globalSave.SRSettings[0]) {
        getId('SRMain').textContent = `Ended everything for ${format(player.buildings[6][1].true)} Cosmon`;
    }
    endReset();
    numbersUpdate();
};
...
const endReset = () => {
    ...
    const income = player.buildings[6][1].true;
    player.cosmon[1].current += income;
    player.cosmon[1].total += income;
    player.inflation.ends++;
    player.inflation.vacuum = false;
    player.time.export[1] = 0;
    player.time.export[2] = 0;
    player.challenges.active = null;
...
export const assignMilestoneInformation = (index: number, stageIndex: number) => {
    ...
    if (player.inflation.vacuum) {
        if (stageIndex === 1) {
            if (index === 0) {
                const scale = player.tree[1][3] >= 2 ? '1e24' : '1e32';
                pointer.need[0].setValue(scale).power(level).multiply(scale);
...
const awardVoidReward = (index: number): void => {
    ...
    if (index === 1) {
        ...
        if (player.vaporization.clouds > 1e12 && player.accretion.rank <= 1) { progress++; }
    } else if (index === 3) {
...
const exitChallengeAuto = (): boolean => {
    const old = player.challenges.active;
    if (old === null || player.strangeness[5][6] < (player.inflation.vacuum ? 1 : 2)) { return false; }
    const info = global.challengesInfo[old];
    if (player.time[info.resetType] <= info.time) { return false; }
