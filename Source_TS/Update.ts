...
export const numbersUpdate = () => {
    ...
    if (tab === 'stage') {
        ...
        if (subtab.settingsCurrent === 'Settings') {
            ...
            getId('offlineStat').textContent = format(player.time.offline, { type: 'time', short: true });
            getId('maxOfflineStat').textContent = format(maxOfflineTime(), { type: 'time', short: true });
            getId('maxExportStat').textContent = format(exportMultiplier() * maxTimeExp / 86400 / 1e12 ** player.strangeness[5][10]);
            getId('maxExportTime').textContent = format(maxTimeExp, { type: 'time', short: true });
            if (active === 1) {
...
export const visualUpdate = () => {
    ...
    if (tab === 'stage') {
        ...
        if (trueSubtab === 'Upgrades') {
            if (player.inflation.vacuum) {
                getId('researchAuto1').style.display = player.researchesExtra[1][2] >= 1 ? '' : 'none';
                getId('researchAuto2').style.display = player.researchesExtra[1][2] >= 2 ? '' : 'none';
            }
            if (active === 1) {
...
export const getUpgradeDescription = (index: number, type: 'upgrades' | 'researches' | 'researchesExtra' | 'researchesAuto' | 'ASR' | 'elements') => {
    ...
    if (type === 'upgrades') {
        ...
        getId('upgradeText').textContent = `${pointer.name[index]}.`;
        getId('upgradeEffect').textContent = pointer.effectText[index]();
        getId('upgradeCost').textContent = player[type][stageIndex][index] >= pointer.max[index] ? 'Maxed.' :
            stageIndex === 4 && type === 'researches' && global.collapseInfo.unlockR[index] > player.collapse.mass && player.buildings[5][3].true < 1 ? `Unlocked at ${format(global.collapseInfo.unlockR[index])} Mass.` :
        ...
        getId('upgradeText').textContent = `${pointer.name[index]}.`;
        getId('upgradeEffect').textContent = pointer.effectText[index]();
        if (player.researchesAuto[index] >= pointer.max[index]) {
            getId('upgradeCost').textContent = 'Maxed.';
        } else {
            const autoStage = pointer.autoStage[index][player.researchesAuto[index]];
        ...
        const autoIndex = Math.min(player.ASR[stageIndex] + 1, pointer.max[stageIndex]);
        ...
        getId('upgradeCost').textContent = player.ASR[stageIndex] >= pointer.max[stageIndex] ? 'Maxed.' :
            stageIndex === 3 && player.accretion.rank < 1 ? "Cannot be created while in 'Ocean world'." :
            `${format(pointer.cost[stageIndex])} ${global.stageInfo.priceName}.`;
    }
};
...
export const format = (input: number /*| overlimit*/, settings = {} as { digits?: number, type?: 'number' | 'input' | 'time' | 'income', padding?: boolean, short?: boolean }): string => {
...
export const stageUpdate = (extra = 'normal' as 'normal' | 'soft' | 'reload') => {
    ...
    const hoverMessage = `${global.mobileDevice ? 'Touch' : global.screenReader ? 'Focus' : 'Hover'} to see.`;
    for (const text of ['upgrade', 'element']) {
        getId(`${text}Text`).textContent = hoverMessage;
        getId(`${text}Effect`).textContent = hoverMessage;
    ...
    if (extra === 'reload') {
        ...
        getId('strangenessEffect').textContent = hoverMessage;
        getId('strangenessCost').textContent = 'Strange quarks.';
        getId('milestonesMultiline').innerHTML = `<p class="orchidText">Requirement: <span class="greenText">${hoverMessage}</span></p><p class="darkvioletText">Unlock: <span class="greenText">${hoverMessage}</span></p>`;
