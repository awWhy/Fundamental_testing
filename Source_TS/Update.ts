...
export const numbersUpdate = (ignoreOffline = false) => {
    ...
    if (tab === 'stage') {
        if (subtab === 'Structures') {
            ...
            if (active === 1) {
                ...
                if (active === 4) {
                    ...
                    const stable = vacuum || (player.tree[0][4] >= 1 && player.tree[1][9] >= 1 && challenge !== 1);
                    if (stable) {
                    ...
                    if (!vacuum && (!stable || !player.toggles.mergeType)) {
                        const requirement = calculateEffects.mergeRequirement();
                        getId('reset0Button').textContent = player.buildings[5][3].true >= requirement && player.progress.main >= 15 ? 'Change Vacuum state to true' : `Requires ${format(requirement)} Galaxies`;
                    }
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    if (i === 0) {
                        if (challenge !== null) {
                            ...
                            if (player.tree[1][9] < 1) {
                                lockText = 'Requires true Vacuum state';
                            } else if (player.tree[0][4] < 1) {
                                lockText = "Requires 'Void Milestones'";
                            }
                        }
                        const trueID = getId('verse0True');
                        const otherTypes = player.verses[0].other;
                        const relevant = player.progress.main < 24 || (!player.inflation.vacuum && (player.tree[0][4] < 1 || player.tree[1][9] < 1 || challenge === 1)) ? player.verses[0].true : calculateEffects.trueVerses();
                        trueID.textContent = `[${format(relevant, { padding: 'exponent' })}${global.versesInfo.true !== relevant ? ` + ${format(global.versesInfo.true - relevant, { padding: 'exponent' })}` : ''}]`;
                        trueID.dataset.title = `True: ${format(player.verses[0].true)}${otherTypes[2] < 1 ? '' : ` + False: ${format(otherTypes[2])}`}${otherTypes[0] < 1 ? '' : ` + Void: ${format(otherTypes[0])}`}${otherTypes[1] < 1 ? '' : ` + Supervoid: ${format(otherTypes[1])}`}${
                            player.progress.main >= 25 ? ` | Types: ${format(global.versesInfo.types)}` : ''}`;
                        ...
                        const softcap = Math.max(production * 600, 1);
                        const resource = player.cosmon[2].current;
                        if (resource >= softcap) { production = ((resource / softcap) ** 4 + production / softcap) ** 0.25 * softcap - resource; }
                    }
                    ...
                    const cost = Math.ceil(calculateVerseCost(i));
                    getId(`verse${i}`).classList[cost <= (i === 0 ? mergeScore : global.versesInfo.types) ? 'add' : 'remove']('availableB');
                    getId(`verse${i}Btn`).textContent = `Need: ${format(cost, { padding: i !== 1 })} ${i === 0 ? 'Merge score' : 'Universe types'}`;
                    getId(`verse${i}BuyX`).textContent = '1';
                }

                if (player.darkness.unlocked[vacuum ? 1 : 0]) {
                ...
                if (vacuum || (player.tree[0][4] >= 1 && player.tree[1][9] >= 1 && challenge !== 1)) {
                    const merge = player.merge;
            ...
            getId('reset2Button').textContent = player.darkness.energy >= 1000 || global.versesInfo.true >= 1 ? `Big ${player.darkness.energy >= 1000 ? 'Rip' : 'Crunch'} for ${format(cosmonGain, { padding: true })} Cosmons` : 'Requires a self-made Universe';
            if (!vacuum && (active < 4 || active >= 6)) {
                const dark = active === 6 && player.darkness.unlocked[0] && player.tree[0][4] >= 1;
                getId('stageReward').textContent = format(calculateEffects.strangeGain(false, active !== 6 || player.tree[1][9] < 1), { padding: true });
        ...
        if (subtab === 'Matter') {
            ...
            const mainGain = interstellar ? global.strangeInfo.strange0Gain : calculateEffects.strangeGain(false, active !== 6 || player.tree[1][9] < 1);
        ...
        if (subtab === 'Matter') {
            if (getId('cosmon2InfoMain').style.display !== 'none') {
                let production = assignBuildingsProduction.verse1() * global.inflationInfo.globalSpeed;
                const softcap = Math.max(production * 600, 1);
                const current = player.cosmon[2].current;
                getId('cosmon2InfoSoft').textContent = format(0.25);
                getId('cosmon2InfoStart').textContent = format(softcap, { padding: true });
                getId('cosmon2InfoPre').textContent = format(production, { padding: true });
                if (current >= softcap) { production = ((current / softcap) ** 4 + production / softcap) ** 0.25 * softcap - current; }
                getId('cosmon2InfoPost').textContent = format(production, { padding: true });
            }
            ...
            getQuery('#cosmon2 p').textContent = format(player.cosmon[2].current, { padding: true });
            getId('inflatonGainTrue').textContent = format((!vacuum && (player.tree[0][4] < 1 || player.tree[1][9] < 1 || challenge === 1) ? player.verses[0].true : calculateEffects.trueVerses()) + 1, { padding: 'exponent' });
        ...
        if (subtab === 'Settings') {
            ...
            if (challenge === null || global.challengesInfo[challenge].resetType === 'stage') {
                ...
                if (exportReward[2] > 0 || (vacuum ? player.strangeness[5][8] >= 1 : (player.darkness.unlocked[0] && player.tree[0][4] >= 1 && player.tree[1][9] >= 1))) { exportTypes.push(`<span class="greenText">${format(exportReward[2] / claimPer * conversion, { padding: true })}</span> Strangelets`); }
            }
            ...
            getId('warpButton').textContent = `${globalSave.developerMode ? 'Dev' : Math.floor(player.time.offline / (60_000 * Math.min(7 - player.tree[0][5], 6)))} Warps`;
            ...
            getQuery('#trueUniversesLow > span').textContent = player.inflation.ends[1] >= 1 ? format(player.verses[0].lowest[0]) : 'Infinity';
            ...
            getId('cosmon2StatTotal').textContent = format(player.cosmon[2].total, { padding: true });
        }
    }
...
export const visualUpdate = (ignoreOffline = false) => {
    ...
    if (tab === 'stage') {
        if (subtab === 'Structures') {
            ...
            if (!vacuum) { getQuery('#stageRewardNew > span:last-of-type').textContent = global.strangeInfo.name[active === 6 && player.tree[1][9] >= 1 ? 1 : 0]; }
            if (active === 1) {
                ...
                if (vacuum) {
                    getId('building5').style.display = player.elements[26] >= 1 && (player.tree[1][9] >= 1 || player.challenges.active !== 0) ? '' : 'none';
                    getId('mainCap').style.display = player.upgrades[4][0] === 1 ? '' : 'none';
                }
                setRemnants();
            } else if (active === 5) {
                const stable = vacuum || (player.tree[0][4] >= 1 && player.tree[1][9] >= 1 && player.challenges.active !== 1);
                ...
                if (!vacuum) {
                    const showTrue = stable && player.toggles.mergeType;
                    getId('mergeTrue').style.display = showTrue ? '' : 'none';
                    getId('mergeFalse').style.display = !showTrue ? '' : 'none';
                    getId('mergeSwap').style.display = stable ? '' : 'none';
        ...
        if (subtab === 'Upgrades') {
            ...
            if (active === 1) {
                ...
                const galaxy = player.researchesExtra[5][0] >= 1;
                const unlocked = vacuum ? player.accretion.rank >= 7 : player.tree[1][9] >= 1;
                ...
                const stable = player.darkness.unlocked[vacuum ? 1 : 0];
            ...
            const verses = upgrades[4] === 1 ? 1 + calculateEffects.trueVerses(true) : 0;
        ...
        if (subtab === 'Matter') {
            ...
            const strangeletsUnlocked = vacuum ? strangeness[5][8] >= 1 : player.darkness.unlocked[0];
            ...
            if (vacuum) {
                ...
                getId('strange0Type').textContent = global.strangeInfo.name[(strangeletsUnlocked || player.tree[1][9] >= 1) && active === 6 ? 1 : 0];
                getId('strangeRateType').textContent = global.strangeInfo.name[(strangeletsUnlocked || player.tree[1][9] >= 1) && active === 6 ? 1 : 0];
                ...
                getId('strange10Stage4').style.display = player.tree[1][9] >= 1 ? '' : 'none';
        ...
        if (subtab === 'Matter') {
            ...
            getId('inflation3Tree2').style.display = supervoid[4] >= 2 ? '' : 'none';
            getId('inflation4Tree2').style.display = supervoid[3] >= 3 ? '' : 'none';
            ...
            getQuery('#endMilestone3 > span > span').textContent = format(0.1);
        ...
        if (subtab === 'Settings') {
            ...
            if (player.challenges.active === null || global.challengesInfo[player.challenges.active].resetType === 'stage') {
                ...
                if (exportReward[2] > 0 || (vacuum ? strangeness[5][8] >= 1 : player.darkness.unlocked[0])) { exportTypes.push(format(exportReward[2] / claimPer / 43200, { type: 'income' }).replace(' ', ' Strangelets ')); }
            }
            ...
            getId('autoDMain').style.display = player.verses[0].lowest[0] <= 6 ? '' : 'none';
            ...
            getId('nucleationToggleReset').style.display = player.darkness.unlocked[vacuum ? 1 : 0] ? '' : 'none';
            ...
            const strangeletsUnlocked = vacuum ? player.strangeness[5][8] >= 1 : player.darkness.unlocked[0];
...
export const visualProgressUnlocks = () => {
    ...
    getId('cosmon2').style.display = highest >= 25 ? '' : 'none';
    ...
    getId('inflation10Tree2').style.display = highest >= 24 ? '' : 'none';
    getId('inflationTree3').style.display = highest >= 25 ? '' : 'none';
    ...
    if (specialHTML.bigWindow === 'hotkeys') {
        ...
        if (highest >= 23) { getId('nucleationHotkey').style.display = ''; }
    }
};
...
export const getUpgradeDescription = (type: 'upgrades' | 'researches' | 'researchesExtra' | 'researchesAuto' | 'ASR' | 'elements' | 'strangeness' | 'milestones' | 'inflation') => {
    if (type === 'inflation') {
        ...
        if (level >= pointer.max[index]) {
            ...
            getId('inflationCost').textContent = `${format(cost)} ${stageIndex === 0 ? 'Inflatons' : stageIndex === 1 ? 'Cosmons' : 'Tachyons'}${stageIndex !== 0 ? ', non-refundable' : ''}.${newLevel - level > 1 ? ` [x${newLevel - level}]` : ''}`;
        }
        ...
        if (index === null) {
            ...
            const notEnoughUniverses = stageIndex === 5 && global.mergeInfo.unlockU[index] > calculateEffects.trueVerses(true);
            ...
            getId('upgradeCost').textContent = player.upgrades[stageIndex][index] === 1 ? 'Created.' :
                ...
                notEnoughUniverses ? `Unlocked after ${global.mergeInfo.unlockU[index] - calculateEffects.trueVerses(true)} more ${player.inflation.vacuum} Vacuum based Universes.` :
            ...
            const notEnoughUniverses = stageIndex === 5 && global.mergeInfo[`unlock${type === 'researches' ? 'R' : 'E'}`][index] > calculateEffects.trueVerses(true);
            ...
            if (level >= pointer.max[index]) {
                ...
                getId('upgradeCost').textContent = `Unlocked after ${global.mergeInfo[`unlock${type === 'researches' ? 'R' : 'E'}`][index] - calculateEffects.trueVerses(true)} more ${player.inflation.vacuum} Vacuum based Universes.`;
            } else {
                let newLevels = 1;
                let cost = pointer.cost[index];
...
export const getChallengeDescription = () => {
    ...
    getId('challengeActive').style.display = player.challenges.active === index ? '' : 'none';
    ...
    if (unlocked) {
        const timerActive = player.challenges.active === index || index === 2;
...
export const getChallengeRewards = () => {
    ...
    if (global.lastChallenge[0] === 0) {
        ...
        const rewardText = info.rewardText;
        const needText = info.needText;
        const current = player.challenges.stability;
        getId('stabilityRewardsHead').textContent = 'Completions rewards:';
        ...
        for (let i = 0; i < rewardText.length; i++) {
            ...
            <p><span class="${unlocked ? 'greenText' : 'redText'}">Reward: </span>${current > i ? rewardText[i] : 'Effect is not yet known'}</p></div>`;
        }
    } else {
        const info = global.challengesInfo[2];

        const current = player.verses[0].lowest[0];
        text += `Lowest completion for selected tier is ${player.inflation.ends[1] >= 1 ? format(player.verses[0].lowest[0]) : 'Infinity'}, completed through Big Rips\nMost of rewards are related to ${global.challengesInfo[2].name} automatization</p>`;
        for (let i = info.rewardText.length - 1; i >= 0; i--) {
            const unlocked = current <= i;
            let failText;
            if (!unlocked && global.versesInfo.true > i) {
...
export const stageUpdate = (changed = true, ignoreOffline = false) => {
    ...
    if (!vacuum) {
        ...
        if (active !== 5) { getId('mergeSwap').style.display = 'none'; }
    }
